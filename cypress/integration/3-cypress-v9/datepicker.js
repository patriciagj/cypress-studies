...
describe('Test Datepicker via webdriveruni', () => {
  it('Select date from the datepicker', () => {
      ...
      cy.get('.datepicker-dropdown')
        ...
        .then((currentDate) => {
          if (!currentDate.text().includes(futureYear)) {
            cy.get('.next').first().click() // Selects the correct year.
            selectMonthAndYear()
          }
        })
        .then(() => {
          cy.get('.datepicker-dropdown')
            ...
            .then((currentDate) => {
              if (!currentDate.text().includes(futureMonth)) {
                cy.get('.next').first().click() // Selects the correct month.
                selectMonthAndYear()
              }
            })
        })
    ...
    function selectFutureDay() {
      cy.get('[class="day"]').contains(futureDay).click() // Selects the correct day.
    }
